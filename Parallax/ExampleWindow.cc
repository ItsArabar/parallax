...
ExampleWindow::ExampleWindow(int width, int height) :
	...
	_sky = std::shared_ptr<SDL_Texture>(
			IMG_LoadTexture(_renderer.get(), "sky2.jpg"), SDL_DestroyTexture);
	if (_sky == nullptr)
	...
	_lug = std::shared_ptr<SDL_Texture>(
			IMG_LoadTexture(_renderer.get(), "grass.png"), SDL_DestroyTexture);
	if (_lug == nullptr)
		throw std::runtime_error(
	...
	_airplane = std::shared_ptr<SDL_Texture>(
			IMG_LoadTexture(_renderer.get(), "airplane.gif"),
			SDL_DestroyTexture);
...
void ExampleWindow::render() {
	...
	SDL_Rect mountes_rect { _sky_x, _mountes_y + 70, width(), height() / 2 };
	SDL_Rect mountes_rect2 { _sky_x2, _mountes_y + 70, width(), height() / 2 };
	SDL_Rect tree_rect{ _tree_x, _tree_y + 70, width()/5, height()/4};
	SDL_Rect tree_rect2{ _tree_x2, _tree_y + 70, width()/5, height()/4};
	...
	SDL_Rect lug_rect{ _lug_x, _lug_y - 30, width(), height()/2};
	SDL_Rect lug_rect2{ _lug_x2, _lug_y - 30, width(), height()/2};
